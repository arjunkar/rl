...
can produce a policy within 250 training episodes (policy updates)
which achieves over 10 test rollouts an average reward 
of 495.4 with variance 3.8.
...
def vpg_loss(obs_seq, act_seq, rew_seq, mask_seq):
    ...
    return -(log_probs * weights).sum() / batch_size, weights.sum() / batch_size
